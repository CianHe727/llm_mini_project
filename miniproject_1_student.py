...
def get_sorted_cosine_similarity(embeddings_metadata):
    """
    [修复版] 获取输入句子与类别之间排序后的余弦相似度
    能够自动处理新输入的类别，防止 KeyError
    """
    # 1. 获取当前界面上的类别和输入文本
    # 注意：这里加了 .strip() 去除可能多余的空格
    raw_categories = st.session_state.categories.split(" ")
    categories = [c for c in raw_categories if c.strip()]  # 过滤掉空字符串
    ...
    if embedding_model == "glove":
        ...
        input_embedding = averaged_glove_embeddings_gdrive(text_search, word_index_dict, embeddings, model_type)

        cache_key = "cat_embed_glove_" + model_type
        if cache_key not in st.session_state:
            st.session_state[cache_key] = {}
        cat_embeds = st.session_state[cache_key]

        for i, cat in enumerate(categories):
            # 安全检查：如果是个新词，现算！
            if cat not in cat_embeds:
                cat_embeds[cat] = averaged_glove_embeddings_gdrive(cat, word_index_dict, embeddings, model_type)

            sim_score = cosine_similarity(input_embedding, cat_embeds[cat])
            results.append((i, sim_score))
    ...
    # -------------------------------------------------------
        ...
        input_embedding = get_openai_embeddings(text_search, model_name=model_name)
        ...
        for i, cat in enumerate(categories):
            # 安全检查：如果是个新词，现算！
            if cat not in cat_embeds:
                cat_embeds[cat] = get_openai_embeddings(cat, model_name=model_name)

            sim_score = cosine_similarity(input_embedding, cat_embeds[cat])
            results.append((i, sim_score))

    # -------------------------------------------------------
    # 3. Sentence Transformer 模型处理
    # -------------------------------------------------------
    else:
        model_name = embeddings_metadata["model_name"]
        model_suffix = model_name if model_name else "default"
        cache_key = "cat_embed_transformers_" + model_suffix
        ...
        for i, cat in enumerate(categories):
            # 安全检查：如果是个新词，现算！
            if cat not in cat_embeds:
                cat_embeds[cat] = get_sentence_transformer_embeddings(cat, model_name=model_name)

            sim_score = cosine_similarity(input_embedding, cat_embeds[cat])
            results.append((i, sim_score))

    # 排序并返回
    sorted_cosine_sim = sorted(results, key=lambda x: x[1], reverse=True)
